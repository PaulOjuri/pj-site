...
export const proseComponents: MDXComponents = {
  ...
  a: ({ href, children }) => {
    if (href?.startsWith('/')) {
      return (
        <Link
          href={href}
          className="text-ink underline underline-offset-4 decoration-subtle hover:decoration-accent transition-colors"
        >
          {children}
        </Link>
      )
  ...
  ol: ({ children }) => (
    <ol className="list-decimal list-outside pl-5 text-muted space-y-2 mb-6">
      {children}
    </ol>
  ),
