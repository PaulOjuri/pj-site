...
export function HeroTagline() {
  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      ...
      if (eyebrow) {
        gsap.fromTo(eyebrow, { opacity: 0 }, { opacity: 1, duration: 0.7, delay: 0.1 })
      }

      const cta = containerRef.current?.querySelector('.hero-cta')
      if (cta) {
        gsap.fromTo(cta, { opacity: 0, y: 12 }, { opacity: 1, y: 0, duration: 0.7, ease: 'power2.out', delay: 1.3 })
      }
    }, containerRef)
  ...
  return (
    ...
    >
      <p
        ...
        style={{ color: 'var(--lc-ivory)', marginBottom: '2rem', opacity: 0 }}
      >
      ...
      <div className="hero-cta" style={{ opacity: 0 }}>
        <Link href="/luxciti/enquire" className="lc-btn-on-dark">
